	...
	then
		...
		do
			...
			then
				...
				echo "docker run -itd --name $HOST -h $HOST --network tzddocker -v $SCRIPTSDIR:/mnt/host -v /InterSystems/jrnalt -v  /InterSystems/jrnpri $IMAGENAME:$IMAGETAG /bin/sh 2>&1"
				docker run -itd --name $HOST -h $HOST --network tzddocker -v $SCRIPTSDIR:/mnt/host -v /InterSystems/jrnalt -v  /InterSystems/jrnpri $IMAGENAME:$IMAGETAG /bin/sh 2>&1

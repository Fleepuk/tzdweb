...
# check routing tables!
	...
	then
		echo "docker run -id --name $HOST -h $HOST  --network tzddocker -v $SCRIPTSDIR/$OUTDIR:/mnt/host ${CUSTOMCOMMAND} $IMAGENAME /bin/sh"
		docker run -itd --name $HOST -h $HOST  --network tzddocker -v $SCRIPTSDIR/$OUTDIR:/mnt/host ${CUSTOMCOMMAND} $IMAGENAME /bin/sh 2>&1
	else
		if [[ "$ENTRYPOINT" == "/sbin/pseudo-init" ]]
			then
				echo "docker run -d --name $HOST -h $HOST --network tzddocker -v $SCRIPTSDIR/$OUTDIR:/mnt/host -v /InterSystems/jrnalt -v  /InterSystems/jrnpri ${CUSTOMCOMMAND} $IMAGENAME"
				docker run -d --name $HOST -h $HOST --network tzddocker -v $SCRIPTSDIR/$OUTDIR:/mnt/host -v /InterSystems/jrnalt -v  /InterSystems/jrnpri ${CUSTOMCOMMAND} $IMAGENAME 2>&1
			else
				echo "docker run -d --name $HOST -h $HOST --network tzddocker -v $SCRIPTSDIR/$OUTDIR:/mnt/host ${CUSTOMCOMMAND} $IMAGENAME ${ENTRYPOINT}"
				docker run -d --name $HOST -h $HOST --network tzddocker -v $SCRIPTSDIR/$OUTDIR:/mnt/host ${CUSTOMCOMMAND} $IMAGENAME
			fi
		fi
	fi	
